...
function Game({ level }) {
  ...
  useEffect(() => window.scrollTo(0, 0), []); // scroll to top when mounted

  const isGameOver = () => {
    const allFound = characters.filter((char) => char.found).length
      === levelData.getLevelCharacters(level).length;
  ...
  const verifyResults = async (character) => {
    ...
    if (
      lastClick.x >= coords.x.min
      && lastClick.x <= coords.x.max
      && lastClick.y >= coords.y.min
      && lastClick.y <= coords.y.max
    ) {
  ...
  return (
    <div>
      ...
      <GameInfoContainer>
        <Timer gameOver={gameOver} handleTimerStop={setTime} />
